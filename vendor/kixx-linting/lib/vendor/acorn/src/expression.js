...
pp.parseMaybeUnary = function(refDestructuringErrors, sawUnary, incDec, forInit) {
    ...
    if (this.isContextual("await") && this.canAwait) {
        ...
        const node = this.startNode(), update = this.type === tt.incDec;
        node.operator = this.value;
        node.prefix = true;
        this.next();
...
pp.parseSubscript = function(base, startPos, startLoc, noCalls, maybeAsyncArrow, optionalChained, forInit) {
    ...
    const optional = optionalSupported && this.eat(tt.questionDot);
    ...
    if (computed || (optional && this.type !== tt.parenL && this.type !== tt.backQuote) || this.eat(tt.dot)) {
        ...
        const exprList = this.parseExprList(tt.parenR, this.options.ecmaVersion >= 8, false, refDestructuringErrors);
...
pp.parseExprAtom = function(refDestructuringErrors, forInit, forNew) {
    ...
    switch (this.type) {
        ...
        case tt.name:
            const startPos = this.start, startLoc = this.startLoc, containsEsc = this.containsEsc;
...
pp.parseFunctionBody = function(node, isArrowFunction, isMethod, forInit) {
    const isExpression = isArrowFunction && this.type !== tt.braceL;
